...
*/
let cashBox = document.getElementById("cash"); // fixed the syntax error - added quotes around "cash"
let billBox = document.getElementById("bill"); // fixed the syntax error - added quotes around "bill"
let changeBox = document.getElementById("change");

// Event handlers to be run when the cash or bill value changes
cashBox.addEventListener("change", runTheRegister); // fixed the function name - changed from "runRegister" to "runTheRegister"
billBox.addEventListener("change", runTheRegister); // fixed the function name - changed from "runRegister" to "runTheRegister"
...
function runTheRegister() {
   ...
   try {
     ...
     if (billBox.value > 100) {
       throw new Error("Bill amount cannot exceed 100");
     }
